...
public class BasicResultsPage {
	...
	public BasicResultsPage(List pageResult, Long totalResults, Integer index,
		...
		if (resultLimit != null && totalResults > resultLimit)
			this.totalResults = resultLimit;

		// calculate the number of pages and set last page index
		lastPageIndex = (int) Math.ceil(this.totalResults / pageSize);
		...
		lastResult = Math.min(firstResult + pageSize, this.totalResults);
		results = pageResult;
